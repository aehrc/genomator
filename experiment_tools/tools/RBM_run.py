...
class RBM:
    ...
    def GetAv(self,it_mcmc=0,βs=-1):
        ...
        if (self.anneal_steps > 0)&(self.ep_tot>1):
            β_init = 1
            β_end = 1
            ...
            for t in range(1,self.anneal_steps): # to take into account the update before next for loop
                β += Dβ
    ...
    def fit(self,X,ep_max=0,B=1000,NS=None,it_mcmc=None,output_vcf_file="RBM_output"):
...
def main_RBM(inpt, NS, ep_max=2001, B=1000, it_mcmc=50, gpu=False, dump_output_interval=50,output_vcf_file="RBM_output",Nh=500,lr=0.01):
    # CHOOSE CPU OR GPU
    if not gpu:
    ...
    stamp='TrainingGene10K_5KP0_OOELearning_Nh'+str(Nh)+'_lr'+str(lr)+'_l2'+str(l2)+'_Rdm_NGibbs'+str(NGibbs)
    myRBM.file_stamp = stamp

    # Number of epochs
    #ep_max = 2001
    # Frequency of saving the weights
    fq_msr = dump_output_interval #50
    ...
    fname_gen = 'RBM'+stamp+'.h5' #TrainingGene10K_OOELearning_Nh2000_lr0.001_l20.0_Rdm_NGibbs50.h5'
    ...
    vs,vm,_,_ = RBMGene.Sampling(vinit,it_mcmc=it_mcmc)
...
@click.option('--it_mcmc', type=click.INT, default=50)
@click.option('--gpu', type=click.BOOL, default=False)
@click.option('--dump_output_interval', type=click.INT, default=50)
@click.option('--nh', type=click.INT, default=500)
@click.option('--lr', type=click.FLOAT, default=0.01)
def RBM_run(input_vcf_file, output_vcf_file, number_of_genomes,ep_max,b,it_mcmc,gpu,dump_output_interval,nh,lr):
    print(gpu)
    ...
    ss = main_RBM(s, number_of_genomes,ep_max=ep_max,B=b,it_mcmc=it_mcmc,gpu=gpu,output_vcf_file=output_vcf_file,dump_output_interval=dump_output_interval,Nh=nh,lr=lr)

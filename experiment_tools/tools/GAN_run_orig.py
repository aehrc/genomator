...
def main_GAN(inpt,ag_size,epochs=10001,batch_size=32,gpu_count=0,dump_output_interval=-1,ploidy=2,output_vcf_file="GAN_output",input_vcf_file=None,layer_multiplier=1.0):
    ...
    generator.add(Dense(int((layer_multiplier*df_noname.shape[1])//1.2), input_shape=(latent_size,), kernel_regularizer=regularizers.l2(0.0001), kernel_initializer=initializers.RandomNormal(stddev=0.01), bias_initializer=initializers.Zeros()))
    generator.add(LeakyReLU(alpha=alph))
    generator.add(Dense(int((layer_multiplier*df_noname.shape[1])//1.1), kernel_regularizer=regularizers.l2(0.0001), kernel_initializer=initializers.RandomNormal(stddev=0.01), bias_initializer=initializers.Zeros()))
    generator.add(LeakyReLU(alpha=alph))
    generator.add(Dense(df_noname.shape[1], activation = 'tanh'))

    #Make discriminator
    discriminator = Sequential()
    discriminator.add(Dense(int((layer_multiplier*df_noname.shape[1])//2), input_shape=(df_noname.shape[1],), kernel_regularizer=regularizers.l2(0.0001), kernel_initializer=initializers.RandomNormal(stddev=0.01), bias_initializer=initializers.Zeros()))
    discriminator.add(LeakyReLU(alpha=alph))
    discriminator.add(Dense(int((layer_multiplier*df_noname.shape[1])//3), kernel_regularizer=regularizers.l2(0.0001), kernel_initializer=initializers.RandomNormal(stddev=0.01), bias_initializer=initializers.Zeros()))
    discriminator.add(LeakyReLU(alpha=alph))
...
@click.option('--layer_multiplier', type=click.FLOAT, default=1.0)
def GAN_run(input_vcf_file, output_vcf_file, number_of_genomes,epochs,dump_output_interval,batch_size,gpu_count,layer_multiplier):
    ...
    ss = main_GAN(s, number_of_genomes,input_vcf_file=input_vcf_file,output_vcf_file=output_vcf_file,epochs=epochs,batch_size=batch_size,gpu_count=gpu_count,dump_output_interval=dump_output_interval,ploidy=ploidy,layer_multiplier=layer_multiplier)

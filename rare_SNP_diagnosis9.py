...
from genomator import parse_VCF_to_genome_strings, parse_genome_strings_to_VCF
import json
import click
import pickle
...
def Rare_SNP_analyse(input_vcf_files,trials,degree,output_image_file):
    ...
    for k in sorted(reverse_labels.keys()):
        ...
        for i in reverse_labels[k]:
            ...
            ratio_unique_responses_greater_than_zero = sum(unique_responses)*1.0/len(unique_responses)
            ratio_zero_responses_greater_than_zero = sum(zero_responses)*1.0/len(zero_responses)
            ratio_unique_responses_greater_than_zero /= len(d)
            ratio_zero_responses_greater_than_zero /= len(d)
            with open("privacy_results.txt","a") as f:
                f.write("{},{}\n".format(ratio_unique_responses_greater_than_zero,ratio_zero_responses_greater_than_zero))

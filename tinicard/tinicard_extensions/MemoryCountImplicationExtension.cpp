...
class MemoryCountImplicationExtension : public ImplicationExtension {
private:
...
public:
	...
	MemoryCountImplicationExtension(SatSolver* s, PyObject* binary_queries, int array_width, float exception_space, int not_silent) {
        ...
        int difference_bits = 0;
				for (int k=0; k<array_width_blocks; k++)
					difference_bits += std::popcount((array[i][k] | array[j][k]) ^ masks[k]);
        if (difference_bits > generate_random(exception_space))
          goto iterate_next_j;
	...
	~MemoryCountImplicationExtension() {
	}

...
struct FastCountCompactMemoryImplicationExtensionArraySlice {
	_ULL_** array;
	int offset;
	int size;
};

class FastCountCompactMemoryImplicationExtension : public ImplicationExtension {
private:
	int* block_buffer;
	int loaded_block;
	...
	FastCountCompactMemoryImplicationExtensionArraySlice* array_slices;
	int index_bits;
	_ULL_ index_mask;
	int index_bitshift;

  float exception_space;
  float looseness;
...
public:
	...
	FastCountCompactMemoryImplicationExtension(SatSolver* s, PyObject* binary_queries, int array_width, int index_bits, float exception_space, float looseness, int not_silent);
	~FastCountCompactMemoryImplicationExtension();
	...
	inline void loadBlock(int lit) {
		...
		_ULL_* line = array[oj];
		_ULL_ line_index_mask = (line[0]>>index_bitshift);
    float rand_modifier = (fabs(exception_space)+1)/((float)RAND_MAX);
		for (int k=0; k<array_width_blocks; k++)
			masks[k] = (masks[k] & (~line[k]));
    for (_ULL_ i=0; i<=index_mask; i++) {
      //int exception_space1 = std::popcount(index_mask ^ (index_mask & (line_index_mask|i)));
      int exception_space1 = __builtin_popcountll(index_mask ^ (index_mask & (line_index_mask|i)));
      if (exception_space1 > ceil(fabs(exception_space)))
        continue;
      FastCountCompactMemoryImplicationExtensionArraySlice array_slices_i = array_slices[i];

			if (array_slices_i.array!=NULL) {
 				unsigned int point_index=0;
				for (_ULL_* array_iterator = array_slices_i.array[0]; *array_iterator; array_iterator++, point_index++) {

          int exception_space2 = 0;
          long var2 = wrap(point_index+array_slices_i.offset,array_halflength);
          int rand_exception_limit = exception_space;
          if (exception_space < 0) rand_exception_limit = scramble_with_mod(rand_modifier,(unsigned int)(abs(lit)+abs(var2)+abs(lit)*abs(var2)));
					for (int k=0; k<array_width_blocks; k++) {
						_ULL_ maskk = masks[k];
            //exception_space2 += std::popcount(maskk ^ (maskk & ( (array_slices[i].array[k][point_index])|line[k])));
            //exception_space2 += std::popcount(maskk ^ (maskk & (array_slices_i.array[k][point_index])));
            //exception_space2 += std::popcount(maskk & (~(array_slices_i.array[k][point_index])));
            exception_space2 += __builtin_popcountll(maskk & (~(array_slices_i.array[k][point_index])));
            if (exception_space2 > rand_exception_limit)
							goto FastCountCompactMemoryImplicationExtension_next_iteration;
					}
          if (var2 != lit) // so that a literal does not imply its opposite (indicating a degenerate clause)
            if (scramble(abs(lit)+abs(var2)+abs(lit)*abs(var2)+747474+array_length) / (float)RAND_MAX >= looseness)
              if (var2 != -lit) // prevent unit clauses from implication extension (these play badly with Tinisat)
                block_buffer[loaded_block_size++] = -var2;
					FastCountCompactMemoryImplicationExtension_next_iteration:;
				}
			}
    }
...
FastCountCompactMemoryImplicationExtension::FastCountCompactMemoryImplicationExtension(SatSolver* s, PyObject* binary_queries, int original_array_width, int index_bits, float exception_space, float looseness, int not_silent) : index_bits(index_bits), exception_space(exception_space), looseness(looseness) {
	...
	block_buffer = (int*)calloc(sizeof(int),array_length+2);
	...
	array_slices = (FastCountCompactMemoryImplicationExtensionArraySlice*)calloc( sizeof(FastCountCompactMemoryImplicationExtensionArraySlice), _ULLONE_<<index_bits);
	...
	for (int oj=0; oj<array_length; oj++) {
		...
		for (int k=0; k<array_width_blocks; k++) {
			if (line[k]!=masks[k])
				goto FastCountCompactMemoryImplicationExtension_next_unit_check;
		}
		var1 = wrap(oj,array_halflength);
		s->addImplication(-var1,-var1); // unit clause add
		activities[unwrap(-var1,array_length)]++;
		FastCountCompactMemoryImplicationExtension_next_unit_check:;
	}
...
FastCountCompactMemoryImplicationExtension::~FastCountCompactMemoryImplicationExtension() {
	free(masks);
...
void FastCountCompactMemoryImplicationExtension::resetMasks() {
	for (int k=0; k<array_width_blocks; k++) {

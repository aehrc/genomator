...
struct FastRangedMemoryImplicationExtensionArraySlice {
	_ULL_** array;
	int offset;
	int size;
};

class FastRangedMemoryImplicationExtension : public ImplicationExtension {
private:
	VectorRangeSet* implication_ranges;
	int* block_buffer;
	int* activities;

	FastRangedMemoryImplicationExtensionArraySlice* array_slices;
	int index_bits;
	_ULL_ index_mask;
	int index_bitshift;
...
public:
	...
	FastRangedMemoryImplicationExtension(SatSolver* s, PyObject* binary_queries, int array_width, int index_bits, int not_silent);
	~FastRangedMemoryImplicationExtension();

	inline int getVariableNum() {
		return array_halflength;
	}
	inline int getBlockSize(int lit) {
		return implication_ranges[unwrap(lit, array_length)].data_size;
	}
	inline int getBlockSizeNonZero(int lit) {
		return getBlockSize(lit)!=0;
	}
	inline int* getBlock(int lit) {
		long unwraplit = unwrap(lit, array_length);
		int index=0;
		for (auto it = implication_ranges[unwraplit].buffer.begin(); it!=implication_ranges[unwraplit].buffer.end(); it++) {
			for (int c = (*it).first; c<=(*it).second; c++) {
				block_buffer[index++] = wrap(-c,array_halflength);
			}
		}
    block_buffer[index+1]=0;
		return block_buffer;
	}
	inline int getActivity(int lit) {
		return activities[unwrap(lit, array_length)];
	}
};



FastRangedMemoryImplicationExtension::FastRangedMemoryImplicationExtension(SatSolver* s, PyObject* binary_queries, int original_array_width, int index_bits, int not_silent) : index_bits(index_bits) {
	...
	array_slices = (FastRangedMemoryImplicationExtensionArraySlice*)calloc( sizeof(FastRangedMemoryImplicationExtensionArraySlice), _ULLONE_<<index_bits);
	...
	implication_ranges = new VectorRangeSet[array_length];
			...
			if (array_slices[i].array!=NULL) {
				for (_ULL_* array_iterator = array_slices[i].array[0]; *array_iterator; array_iterator++) {
					...
					if (point_index+array_slices[i].offset==oj) {
						s->addImplication(-var1,-var2); // unit clause add
						activities[unwrap(-var1,array_length)]++;
					}
					if (point_index+array_slices[i].offset>oj) {
						uvar1 = unwrap(-var1,array_length);
						uvar2 = unwrap(-var2,array_length);
						implication_ranges[uvar1].add(-uvar2);
						implication_ranges[uvar2].add(-uvar1);
						activities[uvar1]++;
						activities[uvar2]++;
					}
					FastRangedMemoryImplicationExtension_next_iteration:;
				}
			}
		}
		resetMasks();
...
FastRangedMemoryImplicationExtension::~FastRangedMemoryImplicationExtension() {
	delete[] implication_ranges;
	free(block_buffer);
	free(activities);
}


void FastRangedMemoryImplicationExtension::resetMasks() {
	for (int k=0; k<array_width_blocks; k++) {
